/*	@(#)cpp.c	2.1		*/
char	release[] = "@(#)C rel 2.1; UTS rel 1.1.";

# include <stdio.h>
char *xcopy();	/* CXREF */
FILE *outfp;	/* CXREF */
static int ready = 0;	/* CXREF */
int xline;	/* CXREF */
/* C command
*/
...
sayline() {
	if (pflag==0) fprintf(fout, LINEFORM, lineno[ifno], fnames[ifno]);
}
...
prevlf:
		if (speakup) {
			inp=p; dump(); lineno[ifno]=speakup+1; sayline(); speakup=0;
		}
		state=BEG;
		for (;;) {
			if (*p++=='#') return(p);
...
dodef(p) char *p; {/* process '#define' */
	...
	if (*pin=='(') {/* with parameters; identify the formals */
		newf(np->name, lineno[ifno]);	/* CXREF */
...
for (;;) {
	...
	*--inp=SALT; outp=inp; ++flslvl; np=slookup(inp,p,0); --flslvl;
	if (np==defloc) {/* define */
		...
		ref(xcopy( inp, p ), lineno[ifno]);	/* CXREF */
