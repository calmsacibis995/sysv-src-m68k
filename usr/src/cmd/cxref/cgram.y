...
enum_dcl:	   enum_head LC moe_list optcomma RC
			={ $$ = dclstruct($1); }
		|  ENUM NAME
			={  $$ = rstruct($2,0);  stwart = instruct; /*CXREF ref($2, lineno); */ }
		;

enum_head:	   ENUM
			={  $$ = bstruct(-1,0); stwart = SEENAME; }
		|  ENUM NAME
			={  $$ = bstruct($2,0); stwart = SEENAME; /*CXREF ref($2, lineno); */ }
		;
...
moe:		   NAME
		...
		|  NAME ASSIGN con_e
			={  strucoff = $3;  moedef( $1 ); /*CXREF def($1, lineno); */ }
		;
...
str_head:	   STRUCT
		...
		|  STRUCT NAME
			={  $$ = bstruct($2,$1);  stwart=0;  /*CXREF ref($2, lineno); */ }
		;
...
nfdeclarator:	   MUL nfdeclarator		
		...
		|  nfdeclarator LB con_e RB	
			={  bary:
				...
				$$ = bdty( LB, $1, $3 );  }
		|  NAME  		
			={  $$ = bdty( NAME, NIL, $1 );  /*CXREF def($1, lineno); */ }
		|   LP  nfdeclarator  RP 		
			={ $$=$2; }
		;
...
name_list:	   NAME			
			={ ftnarg( $1 );  stwart = SEENAME; /*CXREF ref($1, lineno); */ }
		|  name_list  CM  NAME 
			={ ftnarg( $3 );  stwart = SEENAME; /*CXREF ref($3, lineno); */ }
		| error
		;
...
compoundstmt:	   begin dcl_stat_list
			{
#ifndef LINT
				if( $<intval>2 ) plcstab(blevel);
#endif
			}
			stmt_list RC
			={
#ifndef LINT
			    if( $<intval>2 ) prcstab(blevel);
#endif
			    --blevel;
...
begin:		  LC
			={  setdim(); /*CXREF if(blevel > 1) bbcode(); */
			    if( blevel == 1 ) dclargs();
			    ++blevel;
