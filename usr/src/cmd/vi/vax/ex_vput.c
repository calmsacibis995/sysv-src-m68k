...
static char *sccsid = "@(#)ex_vput.c	2.1	";
#include "ex.h"
#include "ex_tty.h"
#include "ex_vis.h"
	...
	bool didphys;
{
	...
	if (!didphys && (CD || CE)) {
		...
		if (DB) {
			vgoto(WECHO, 0);
			/*
			 * This is tricky.  If CD is as cheap we should use it,
			 * so we don't have extra junk floating around in memory
			 * below.  But if CE costs less we should use it.
			 * The real reason here is that CD is incredibly
			 * expensive on the HP 2626 (1/2 second or more) which
			 * makes ^D scroll really slow.  But the 2621 has a bug
			 * that shows up if we use CE instead of CD, so we make
			 * sure the costs are equal so it will prefer CE.
			 */
			if (costCE < costCD)
				vputp(CE, 1);
			else
